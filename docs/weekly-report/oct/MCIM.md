...
#### 3.2.1 Weighted LeaderRank with Neighbors (WLRN) 

WLRN 算法用来衡量用户的本地重要性。多个社区中的影响最大化问题可以建模如下，WLRN(u~ij~; α, L) 是用户 u~ij~ 的得分，α 和 L 是两个参数。
...
最优状态值函数V*(s)满足贝尔曼最优方程:  $V^∗(s) = \max[r(s,a)+γV_π(s′)]$
